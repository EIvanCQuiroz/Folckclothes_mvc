...
class Usuario extends ActiveRecord{
    ...
    public function validarNuevaCuenta(){
        ...
        if (strlen($this->password) < 8) {//Validacion de password
    ...
    public function comprobarPassNdVerifi($password)
    {
        $resultado = password_verify($password, $this->password);

        if (!$resultado || !$this->confirmado) {
            self::$alertas['error'][] = 'Password incorrecto o problemas con la confirmacion de su cuenta :( ';
        }else{
            return true;
        }
